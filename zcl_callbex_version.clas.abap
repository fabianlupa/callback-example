"! Version
"! <p>
"! Implementation copied from abap-log <em>ZCL_ALOG_VERSION</em>.
"! https://github.com/flaiker/abap-log/blob/d08e9c1856f4f25c868baf026c55c8c0215a00bd/src/zcl_alog_version.clas.abap
"! </p>
CLASS zcl_callbex_version DEFINITION
  PUBLIC
  FINAL
  CREATE PRIVATE.
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS:
      ...
      "! @raising zcx_callbex_illegal_argument | iv_version does not have the required format
      of IMPORTING iv_version        TYPE string
         RETURNING VALUE(ro_version) TYPE REF TO zcl_callbex_version
         RAISING   zcx_callbex_illegal_argument,
      ...
      "!                        </ul>
      "! @raising zcx_callbex_argument_null | io_version_a and io_version_b cannot be null
      compare IMPORTING io_version_a     TYPE REF TO zcl_callbex_version
                        io_version_b     TYPE REF TO zcl_callbex_version
              RETURNING VALUE(rv_result) TYPE i
              RAISING   zcx_callbex_argument_null.
    METHODS:
      ...
      "! @raising zcx_callbex_argument_null | io_version cannot be null
      compare_to IMPORTING io_version       TYPE REF TO zcl_callbex_version
                 RETURNING VALUE(rv_result) TYPE i
                 RAISING   zcx_callbex_argument_null.
    CLASS-DATA:
      ...
      go_current     TYPE REF TO zcl_callbex_version READ-ONLY.
    DATA:
...
CLASS zcl_callbex_version IMPLEMENTATION.
  ...
  METHOD compare.
    ...
    IF io_version_a IS NOT BOUND OR io_version_b IS NOT BOUND.
      RAISE EXCEPTION TYPE zcx_callbex_argument_null
        EXPORTING
  ...
  METHOD of.
    IF NOT matches( val = iv_version regex = `\d+\.\d+\.\d+(-.+)?` ).
      RAISE EXCEPTION TYPE zcx_callbex_illegal_argument
        EXPORTING
